...
@section('index_header')
    ...
    <div class="cms-options">
        <div class="cms-options-title-action">
            ...
            {!! link_to_route('admin.photos.create', 'Add', null, ['class' => 'cms-options-action btn btn-lg btn-primary']) !!}
        </div>
...
@section('index_content')
    <span id="tableName" style="display: none; visibility: hidden;">photos</span>
            ...
            <table id="sortable-table" class="table">
                ...
                @foreach($items as $item)
                    <tr id="{{ $item->id }}" class="{{ $item->deleted_at ? 'danger' : ''}}">
                        <td>
                            <img src="{{ Image::load('gallery/' . $item->image, ['h' => 60]) }}" alt="">
                        </td>
                        <td>{{$item->caption}}</td>
                        ...
                        <td class="cms-column-actions">
                            <div class="btn-group btn-group-xs cms-table-actions">
                                <a href="{{ route('admin.photos.edit', $item->id) }}" type="button" class="btn btn-default"><span class="entypo entypo-pencil"></span></a>
                                {!! Form::open(['method' => 'DELETE', 'route' => ['admin.photos.destroy', $item->id], 'onsubmit' => "return confirm('Are you sure you want to DELETE this item?')"]) !!}
